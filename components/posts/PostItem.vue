  ...
  <div class="blog-loop">
    <div class="blog-loop__img">
      <nuxt-link :to="postUrl">
        <img
          ...
          alt="Genova: bilancio immobiliare del primo semestre 2022"
          loading="lazy"
      /></nuxt-link>
    </div>
    <div class="blog-loop__content">
      <header class="blog-loop__header">
        ...
        <span> In </span>
        <div
          class="blog-loop__category"
          v-if="post.post_tags.data && post.post_tags.data.length"
        >
          <nuxt-link
            v-for="(tag, index) in post.post_tags.data"
            :key="tag.id"
            :to="`/tag/${tag.attributes.slug}`"
          >
            {{ tag.attributes.title }}
            <span v-if="post.post_tags.data.length - 1 !== index">, </span>
          </nuxt-link>
        </div>
      </header>
